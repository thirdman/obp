...
class InvoiceSection extends Component {
				...
				{loaded &&
					<Section title={'Settings'}>
						<ContentItem
							...
							title="Automatically send invoices to QuickBooks when created"
	...
	fetchData() {
		const { currentOrg = {} } = this.context.store.app;
		this.loading('Getting QuickBooks accounting codes ....');
		this.safePromise(xeroClient.request(
			'post',
			`organisations/${currentOrg.id}/qb/accountCodes`,
	...
	updateToken(newToken) {
		...
		this.safePromise(xeroClient.request(
			'put',
			`organisations/${currentOrg.id}/accessTokens?api=qb`,
			token,
			currentOrg,
			{ newToken }
		))
