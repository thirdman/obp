...
class InvoiceSection extends Component {
				...
				{loaded &&
					<Section title={'Settings'}>
						<ContentItem
							...
							title="Automatically send invoices to Sage when created"
	...
	fetchData() {
		const { currentOrg = {} } = this.context.store.app;
		this.loading('Getting sSage accounting codes ....');
		this.safePromise(xeroClient.request(
			'post',
			`organisations/${currentOrg.id}/sage/accountCodes`,
	...
	updateToken(newToken) {
		...
		this.safePromise(xeroClient.request(
			'put',
			`organisations/${currentOrg.id}/accessTokens?api=sage`,
			token,
			currentOrg,
			{ newToken }
		))
