...
class InvoiceSection extends Component {
				...
				{loaded &&
					<Section title={'Settings'}>
						<ContentItem
							...
							title="Automatically send invoices to FreshBooks when created"
	...
	fetchData() {
		const { currentOrg = {} } = this.context.store.app;
		this.loading('Getting FreshBooks accounting codes ....');
		this.safePromise(xeroClient.request(
			'post',
			`organisations/${currentOrg.id}/fb/accountCodes`,
	...
	updateToken(newToken) {
		...
		this.safePromise(xeroClient.request(
			'put',
			`organisations/${currentOrg.id}/accessTokens?api=fb`,
			token,
			currentOrg,
			{ newToken }
		))
